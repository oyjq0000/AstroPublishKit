...
<Callout title="Release principle" tone="tip">
  A successful content change should pass the same checks locally and in CI, then produce the same static output for
...
<Accordion summary="Why not add a database now?">
  Static publishing already covers the first release goals. A database would add deployment, security and migration work
  without improving the core writing path.
</Accordion>
