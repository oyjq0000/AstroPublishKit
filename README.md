...
- `faq` is an optional non-empty list of plain-text `question` / `answer` pairs. The same items render the visible FAQ and `FAQPage` JSON-LD at build time; when the field is absent, neither output is generated. This is semantic Schema.org markup, not a promise of a Google FAQ rich result.
...
Pagefind is generated by `npm run build`. Article titles, descriptions, body content, Quick Answer blocks and reader-visible FAQ content are searchable. Related Posts, Previous / Next navigation, comments and share controls are excluded with `data-pagefind-ignore` so another article's title or UI chrome cannot create a false match.

`npm run dev` is the fast writing preview, but the complete static search index should be tested with:
...
v0.3.1 keeps the v0.3.0 article experience and hardens two production-validated boundaries: JSON-LD is serialized safely for HTML script elements, and Pagefind excludes cross-article navigation plus non-content UI from the current article's index body. Redirect automation is still not included.

See **[Feature matrix](feature-matrix.md)** for current implementation status and later candidates.
