...
test("posts sort newest first without mutating the input", () => {
  ...
  assert.deepEqual(posts.map((item) => item.id), ["old", "new"]);
});
...
test("category and tag filters use exact labels", () => {
  ...
  assert.deepEqual(filterPostsByCategory(posts, "Engineering").map((item) => item.id), ["one"]);
  assert.deepEqual(filterPostsByTag(posts, "Web").map((item) => item.id), ["one", "two"]);
